...
class ExtractorTests(unittest.TestCase):
    def test_google_maps_requires_coordinates(self) -> None:
        ...
        self.assertEqual(
            result["quarantine_cafes"][0]["reason_code"], "missing_required_field"
        )
        self.assertIn(
            "missing required field: latitude",
            result["quarantine_cafes"][0]["validation_errors"],
        )
        self.assertIn(
    ...
    def test_google_maps_builds_photos_and_hours(self) -> None:
        ...
        self.assertEqual(
            result["cafes"][0]["service_options"], ["Outdoor seating", "Delivery"]
        )
        self.assertEqual(
            result["cafes"][0]["highlights"], ["Great coffee", "Great dessert"]
        )
        self.assertEqual(result["cafes"][0]["popular_for"], ["Lunch", "Dinner"])
        self.assertEqual(result["cafes"][0]["offerings"], ["Coffee", "Quick bite"])
        self.assertEqual(
            result["cafes"][0]["dining_options"], ["Breakfast", "Lunch", "Dinner"]
        )
        ...
        self.assertEqual(
            result["cafes"][0]["planning"], ["Accepts reservations"]
        )
        self.assertEqual(
            result["cafes"][0]["payments"], ["Credit cards", "NFC mobile payments"]
        )
        self.assertEqual(result["cafes"][0]["children"], ["Good for kids", "Kids' menu"])
        self.assertEqual(
            result["cafes"][0]["parking"], ["Paid street parking", "Plenty of parking"]
        )
    ...
    def test_tiktok_unmatched_records_are_quarantined(self) -> None:
        ...
        self.assertEqual(result["cafes"], [])
        self.assertEqual(
            result["quarantine_influencer_reviews"][0]["reason_code"],
    ...
    def test_run_extraction_writes_contract_files_and_detects_conflicts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            reviews = json.loads(
                (output_dir / "influencer_reviews.json").read_text(encoding="utf-8")
            )
            quarantine_cafes = json.loads(
                (output_dir / "quarantine_cafes.json").read_text(encoding="utf-8")
            )
            quarantine_reviews = json.loads(
                (output_dir / "quarantine_influencer_reviews.json").read_text(
                    encoding="utf-8"
                )
            )
            ...
            self.assertTrue(
                any(item["reason_code"] == "dedupe_conflict" for item in quarantine_cafes)
            )
            self.assertTrue(
                any(
                    item["reason_code"] == "schema_mismatch"
                    for item in quarantine_reviews
                )
            )
